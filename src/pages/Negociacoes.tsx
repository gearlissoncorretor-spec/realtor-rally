    ...
    try {
      ...
      const { data: createdSale, error } = await (supabase as any).rpc('convert_negotiation_to_sale', {
        ...
        p_sale_data: {
          ...
          notes: data.notes || '',
          pagos: data.pagos ?? 0,
          ano: data.ano ?? (data.sale_date ? new Date(data.sale_date + 'T00:00:00').getFullYear() : undefined),
          mes: data.mes ?? (data.sale_date ? new Date(data.sale_date + 'T00:00:00').getMonth() + 1 : undefined),
      ...
      toast({
        ...
        description: error.message || "Não foi possível converter a negociação em venda. Verifique os dados e tente novamente.",
        variant: "destructive"
      });
    }
  };
