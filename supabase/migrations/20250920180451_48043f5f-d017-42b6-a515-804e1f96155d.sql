...
ALTER TABLE public.profiles ADD COLUMN role user_role DEFAULT 'cliente'::user_role;
...
BEGIN
  ...
  ELSE
    default_role := 'corretor'::user_role;
...
SET search_path TO 'public'
AS $function$
  SELECT COALESCE(role, 'cliente'::user_role)
  FROM public.profiles
  WHERE id = user_id;
$function$;

-- Create function to get default screens by role
CREATE OR REPLACE FUNCTION public.get_default_screens_by_role(user_role_input user_role)
RETURNS TEXT[]
LANGUAGE sql
STABLE
AS $function$
